...
| File | What it is |
|---|---|
| `Boston_Housing_Price_Predictor.ipynb` | The full analysis: data cleaning, EDA, feature engineering, modeling, evaluation |
| `boston.csv` | The dataset |
| `Boston_Housing_Dashboard.xlsx` | An Excel dashboard version of the findings — KPI cards, correlation chart, scatter plots, all built on live formulas |
| `Boston Housing Project.pptx` | A presentation walking through the project end to end, from raw data problems to the deployed app |
| `Boston_House_Prediction/app.py` | The Streamlit app source — trains and serves predictions live |
...
- **Predicted-vs-actual** and **residual plots** to visually check for systematic errors, not just a single summary statistic
...
The app trains its Gradient Boosting model fresh on startup (cached after first load) rather than loading a pickled model file — this avoids scikit-learn version mismatches between the local environment and the deployment server.
...
Keep `boston.csv` in the same folder as whichever file is loading it — both the notebook and the app use a relative path.
